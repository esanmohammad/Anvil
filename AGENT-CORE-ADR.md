...
- **Phase 5** — `6835167` — _claude-runner convergence_ — `single-shot.ts` lands in `agent-core/src/`. `knowledge-core/src/claude-runner.ts` becomes a 13-line re-export shim. All 71 knowledge-core tests pass through the shim unchanged. Env-var aliasing works: `CODE_SEARCH_LLM_*` legacy vars resolve with one-time deprecation warnings to stderr; canonical `ANVIL_LLM_*` vars take precedence silently. **Deviation from plan §5.2:** the plan envisioned `runLLM` wrapping `LanguageModel.invoke()` via `ProviderRegistry`. Reality: the 7 adapters only implement legacy `ModelAdapter.run(config, output)`; none have an `invoke()` method yet. Adding `invoke()` to all 7 (or building a `legacyAdapterToLanguageModel` bridge) is a separate larger refactor. Phase 5 ships **lift-and-shift** — the original subprocess + HTTP transport logic moves verbatim into `single-shot.ts` with the new env-var resolver layered on top. Both paths produce the same `ClaudeResult` shape so callers see no difference. The `LanguageModel`-unification path can be a future phase ("Phase 12+") once adapters grow native `invoke()` impls. Plan §5 should be amended to reflect this two-step approach.
- **Phase 6** — `b993b98` — _hoist `cli/src/agent/`_ — All 9 files moved to `packages/agent-core/src/agent/`; `cli/src/agent/` directory deleted. **Plan correction:** ADR §3.2 (and the original audit memo) said `cli/src/commands/learn.ts` was the sole consumer. Re-grep shows `learn.ts` defines its own local `spawnAgent` function — it does NOT import from `cli/src/agent/`. Net effect: `cli/src/agent/` had **zero external consumers** in cli today, so the move had zero blast radius for cli. Plan §6.6 risk #1 ("orchestrator is the heaviest consumer") was based on outdated info — orchestrator never imported from `agent/` either. Verified via runtime smoke that all expected exports (`AgentManager`, `spawnAgent`, `StreamParser`, `OutputBuffer`, `RestartPolicy`, `TimeoutGuard`, `StageValidator`, `STAGE_TIMEOUT_DEFAULTS`, `createDefaultConfig`) resolve from `@anvil/agent-core`. Note: cli's `resilience/validator-registry.ts` exports an _interface_ also named `StageValidator` but it stays in cli, no collision.
- **Phase 7** — `c84395b` — _cost table integration_ — Ships:
    - `packages/agent-core/scripts/refresh-cost-table.mjs` — fetches LiteLLM `model_prices_and_context_window.json` (Apache-2.0)
    - `packages/agent-core/src/data/model-prices.json` — vendored snapshot (1.4 MB, 2,688 model entries; **plan §7 estimated ~150 KB — outdated**)
    - `packages/agent-core/src/cost.ts` — `getModelPricing`, `getDetailedPricing`, `calculateCost`, `resetCostTable` exports + `MODEL_ALIASES` bridge for Anvil short names (`sonnet`/`opus`/`haiku`)
    - Build script copies JSON into `dist/data/` so the loader resolves at runtime via `import.meta.url + readFileSync` (avoids tsconfig drama with JSON imports)
  Verified: `sonnet`/`opus`/`haiku`/`gpt-4o`/`gemini-2.5-pro`/etc. resolve correctly; unknown models return `null`; `calculateCost` handles cache token columns.
  **Pricing shifts surfaced from refresh:** `opus` was hand-coded `[15, 75]` (claude-3-opus era) — LiteLLM's `claude-opus-4-7` is `[5, 25]`. `haiku` was `[0.25, 1.25]` — LiteLLM's `claude-haiku-4-5` is `[1, 5]`. `o3` was `[10, 40]` — now `[2, 8]`. Hand-coded prices were stale; cost-table refresh is exactly the value-add this phase brings.
  **Plan deviation (documented):** plan §7.2 step 4 calls for migrating each adapter's `getModelPricing` to delegate to the central loader. Doing so would change cost-calculation behavior for existing pipelines (the price shifts above), and there are no adapter tests today to characterize the regression. **Phase 7 ships the loader as a pure addition** — adapters keep their hand-coded PRICING tables. Per-adapter migration deferred to a follow-up phase that can be done with proper testing/coverage. Plan §7 should be amended to call out this two-step approach.
- **Phase 8** — `df01f7b` — _test migration_ — `claude-runner.test.ts` (9 tests) moved from `knowledge-core/src/__tests__/` to `agent-core/src/__tests__/single-shot.test.ts`. Imports rewritten from `@anvil/knowledge-core` to `@anvil/agent-core` (both static `from '...'` and dynamic `import('...')` forms). **Watch-out:** `tsc -b` does NOT delete stale `dist/__tests__/*.test.js` artifacts when source files are removed; first run after the move had to clean `knowledge-core/dist/__tests__/claude-runner.test.*` manually. After cleanup: knowledge-core 62/62 (was 71), agent-core 9/9. Total coverage unchanged.
- **Phase 9** — `ae11c60` — _build/CI consolidation_ — Updated `.github/workflows/{ci,release}.yml` to build `@anvil/agent-core` and `@anvil/knowledge-core` BEFORE `cd packages/cli && npx tsc -b`. **Latent CI bug surfaced:** the prior workflow ran `cd packages/cli && npx tsc -b` directly, but cli's tsconfig has no project references, so `@anvil/agent-core` and `@anvil/knowledge-core` types only resolve if their dists already exist. Fresh-checkout CI runs would have failed with "Cannot find module" errors; previous green runs were masked by `actions/setup-node@v4 cache: 'npm'` plus tsbuildinfo state. Fix: explicit ordered build steps + dedicated `Test agent-core` and `Test knowledge-core` steps. The release workflow got the same treatment. **No vendor SDK dedup needed** — Phase 4a's default decision was to keep each adapter using its current transport (`fetch()` or subprocess); no `@anthropic-ai/sdk` / `openai` / `@google/genai` was introduced. Lockfile unchanged. Verified end-to-end via fresh-checkout simulation: deleted all dists + tsbuildinfo, rebuilt in dependency order — all gates pass (agent-core 9/9, knowledge-core 62/62, dashboard 430/436 matching pre-existing baseline). **Out of scope:** mcp's build script processes 26 files instead of the expected 13 because stale `.d.ts` files exist in `code-search-mcp/src/` (likely from a prior `tsc -b --watch` run; mcp's tsconfig has no `declaration: true` so they shouldn't be there). Pre-existing, unrelated, CI-time waste only.
- **Phase 10** — _docs + ADR finalization_ — `packages/agent-core/README.md` rewritten from the Phase-1 stub into a full reference: goals, public API surface (LanguageModel + ModelAdapter + stream-format + single-shot + agent machinery + cost), env-var matrix with aliases, cost-table refresh procedure, file layout, lock-in surface analysis. Plan file `AGENT-CORE-EXTRACT-PLAN.md` got a "Shipped (2026-04-29)" banner pointing here. This bottom-of-§9 entry closes the per-phase ship log.

### 9.1 Plan corrections to back-port (summary)

If the plan ever gets revised from this ADR, these are the substantive fixes:

1. **§3.5 file inventory** — count `cli/src/agent/*` consumers correctly. Plan/audit said `learn.ts`; reality was zero (Phase 6).
2. **§3 Phase ordering** — `registry.ts` cannot move ahead of the 7 adapters because `registerDefaults()` requires them at runtime. Phase 3 ships types only; registry rolls into Phase 4 (Phase 3+4 reconciliation).
3. **§4 risk list** — call out the ESM/`require` trap. The original `registerDefaults()` used `require()` wrapped in try/catch; under `"type": "module"` it silently failed and cli's `ProviderRegistry.getInstance()` returned 0 adapters (Phase 4 fix).
4. **§5 single-shot vision** — the `LanguageModel.invoke()` unification requires every adapter to grow an `invoke()` impl first. Phase 5 ships lift-and-shift; the unification path is a separate phase (Phase 5 deviation).
5. **§7 snapshot size** — plan estimated 150 KB; LiteLLM's catalog has grown to 1.4 MB / 2,688 models (Phase 7).
6. **§7 adapter migration** — migrating each adapter's `getModelPricing` to delegate to the central loader changes existing cost-calculation behavior (visible price shifts on opus/haiku/o3). Defer to a follow-up phase with proper test coverage (Phase 7 deviation).
7. **§9 CI prerequisites** — `cd packages/cli && npx tsc -b` does NOT build dependencies. CI must build `@anvil/agent-core` and `@anvil/knowledge-core` explicitly first (Phase 9).
8. **§1 ProviderName values** — preserve actual values (`'claude'`, `'openai'`, etc.) rather than the plan's sketched `'anthropic-api' | 'anthropic-cli' | …`. Plan was illustrative; rename would cascade (D15).
9. **dashboard scope** — `packages/dashboard/server/{provider-registry.js,adapters/}` is a parallel JS-only stack that does not consume cli's providers. Out of scope for this initiative; future ADR can decide whether to converge (D16).
