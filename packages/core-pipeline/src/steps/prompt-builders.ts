...
import { buildShipUserPrompt } from '../stages/ship.js';
import { budgetPromptContext } from '../utils/context-budget.js';
import { sliceSpecForRefs } from '../utils/engineer-spec-slicer.js';
import { bundleFiles, parseTasks, type ParsedTask } from '../utils/engineer-task-bundler.js';
import { enforceBudget, type PromptSection } from '../utils/prompt-budget.js';
import type { KbManagerLike } from '../storage-like.js';

/**
 * Phase H13 — promoted from
 * `packages/dashboard/server/steps/prompt-builders.ts` into
 * `core-pipeline/src/steps`. Concrete dashboard types
 * (`KnowledgeBaseManager`, `ProjectInfo`) replaced with structural
 * shapes (`KbManagerLike` from Phase G, and a local
 * `PromptBuilderProjectInfo` for the few `repos[]` fields the
 * builders read).
 */

/** Structural slice of dashboard's `ProjectInfo` the builders read. */
export interface PromptBuilderProjectInfo {
  repos: Array<{
    name: string;
    github?: string;
    language?: string;
...
export interface PromptBuilderContext {
  ...
  featureSlug: string;
  projectYaml: string;
  projectInfo: PromptBuilderProjectInfo | null;
