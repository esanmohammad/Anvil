 ...
 *
 * Phase F3 — promoted from `packages/dashboard/server/engineer-spec-slicer.ts`
 * into `core-pipeline/utils` so the cli's specs/build stages can share
 * one canonical slicer with the dashboard. Pure markdown parsing — no
 * fs / network / process side effects.
 */
