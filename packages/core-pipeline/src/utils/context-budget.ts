 ...
 *
 * Phase F5 — promoted from `packages/dashboard/server/context-budget.ts`
 * into `core-pipeline/utils`. Pure logic over `model-catalog` (agent-core),
 * `token-util`, and `structural-truncator` (both already in core-pipeline
 * since Phase D). The dashboard's prompt-builder is the only current
 * consumer; cli's prompt-builder will follow.
 *
 * Estimates token counts, applies priority-based truncation when context
 * exceeds provider limits, and provides usage feedback. Token limits are
 * resolved via `model-catalog` (family rules + overrides + env-var escape
 * hatch) so new model versions work without code changes.
 *
 * The result type for `applyBudget` is named `ContextBudgetResult` to
 * disambiguate from `prompt-budget`'s `BudgetResult` (a different
 * shape — section-based byte-budget enforcer over `token-util`). The
 * dashboard's local `BudgetResult` alias is preserved by the shim at
 * `packages/dashboard/server/context-budget.ts`.
 */
