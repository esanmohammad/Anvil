/**
 * Byte-budget enforcer for agent prompt sections — guards against context
 * regressions.
 *
 * Phase F4 — promoted from `packages/dashboard/server/prompt-budget.ts` into
 * `core-pipeline/utils` so any consumer (cli prompt-builder, dashboard
 * prompt-builder, future tooling) sees the same byte-budget contract on top
 * of `token-util`. Pure: no fs / network / process side effects.
 */
