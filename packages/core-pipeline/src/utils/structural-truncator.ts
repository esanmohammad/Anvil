/**
 * Code-aware truncation — replacement for `smartTruncate`'s middle-cut.
 *
 * `smartTruncate` keeps the first 40% and last 20% of a string, which
 * works fine for prose but mangles code: half a class header here, an
 * orphaned `}` there. This module takes a code blob, identifies
 * imports, top-level declarations, and bodies via the same boundary
 * regexes the chunker uses, then greedy-packs the result inside a
 * token budget while preferring:
 *
 *   imports  >  exported declarations (full body)
 *            >  exported declarations (signature only)
 *            >  non-exported declarations (signature only)
 *
 * Skipped or stripped symbols are summarised with a single trailing
 * marker so the consumer (an LLM stage) can tell something was
 * dropped.
 *
 * Pure string in / string out — no filesystem, no AST library.
 *
 * Phase D3 — promoted from dashboard/server/structural-truncator.ts
 * so cli + dashboard share one canonical truncator. Behavior
 * identical.
 */
