/**
 * Dynamic model tier resolver — resolves model IDs from a discovery
 * result instead of hardcoding them.
 *
 * Promoted to `@esankhan3/anvil-agent-core` in Phase F2 of the dashboard
 * pipeline-runner deprecation plan (`docs/dashboard-pipeline-deprecation-plan.md`).
 * Lives next to the provider adapters because tier-based routing is
 * provider-shaped knowledge — same layer as `model-catalog`. The
 * dashboard's discovery layer (`provider-registry.ts`, FS+exec heavy)
 * stays in dashboard and produces values that satisfy this resolver's
 ...
 * from the cached discovery result.
 */

// ── Structural input types ───────────────────────────────────────────
//
// Defined here (rather than imported from a discovery layer) so the
// resolver depends only on the shape its callers feed it. Dashboard's
// `DiscoveryResult` and `ModelInfo` satisfy these structurally.

/** Capability flags a discovered model may carry. */
export type ResolverCapability = 'agentic' | 'chat' | 'embedding' | 'reranking';

/** Per-model weight class. `'local'` is the zero-cost (Ollama) tier. */
export type ResolverModelWeight = 'fast' | 'balanced' | 'powerful' | 'local';

/** Minimum shape the resolver reads from each discovered model. */
export interface ResolverModel {
  id: string;
  capabilities: ResolverCapability[];
  tier?: ResolverModelWeight;
}

/** Minimum shape the resolver reads from a discovery result. */
export interface ResolverDiscoveryResult {
  models: ResolverModel[];
}

/**
 * Dashboard-UI cost-tier vocabulary. Distinct from
 * `router/model-registry.ts:ModelTier` (which uses the
 * `'local' | 'cheap' | 'premium'` policy vocabulary). Renamed during
 * Phase F2 to avoid a name collision when both types co-exist on the
 * `@esankhan3/anvil-agent-core` public surface.
 */
export type ResolverTier = 'fast' | 'balanced' | 'thorough';

/** @deprecated Use `ResolverTier`. Kept as an alias for back-compat. */
export type ModelTier = ResolverTier;

type WeightClass = ResolverModelWeight;
 ...
 */
const STAGE_WEIGHTS: Record<ResolverTier, Record<string, WeightClass>> = {
  fast: {
    clarify:               'fast',
    requirements:          'fast',
...
let lastDiscoveryResult: ResolverDiscoveryResult | null = null;
 ...
 */
export function setDiscoveryResult(result: ResolverDiscoveryResult): void {
 ...
 */
function findModelForWeight(weight: WeightClass, models: ResolverModel[]): string | null {
  if (weight === 'local') {
