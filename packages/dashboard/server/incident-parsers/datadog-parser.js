...
// ── parseDatadogAlert ───────────────────────────────────────────────────
export function parseDatadogAlert(raw) {
    if (!isObject(raw)) {
    ...
    const externalId = asString(alert.id) ??
    ...
    const occurredAt = toIso(alert.created_at) ??
...
function pickAlert(raw) {
    if (isObject(raw.alert))
        return raw.alert;
    if (isObject(raw.data) && isObject(raw.data.alert)) {
        return raw.data.alert;
    }
    return raw;
}
...
// ── parseDatadogSpan ────────────────────────────────────────────────────
export function parseDatadogSpan(raw) {
    if (!isObject(raw)) {
    ...
    const externalId = asString(span.trace_id) ??
    ...
    const meta = isObject(span.meta) ? span.meta : {};
    const resourceName = asString(span.resource_name) ??
        asString(span.resource) ??
        asString(span.name);
    const serviceName = asString(span.service) ?? asString(meta['service.name']);
    const title = asString(meta['error.type']) ??
        asString(meta['error.message']) ??
        resourceName ??
        serviceName ??
        'Untitled Datadog span';
    const summary = asString(meta['error.message']) ??
    ...
    const occurredAt = toIso(span.start) ??
        toIso(span.start_time) ??
        toIso(span.timestamp) ??
        new Date(0).toISOString();
    const url = asString(span.url) ??
...
function pickSpan(raw) {
    if (isObject(raw.span))
        return raw.span;
    if (isObject(raw.data) && isObject(raw.data.span)) {
        return raw.data.span;
    }
...
function buildFailingSymbol(stackTrace, resourceName) {
...
function parseFirstStackFrame(stackTrace) {
    for (const rawLine of stackTrace.split(/\r?\n/)) {
        ...
        if (m)
            return { function: m[1], file: m[2], line: Number(m[3]) };
        // v8 without function: at file:line:col
        m = /^at\s+(.+?):(\d+)(?::\d+)?$/.exec(line);
        if (m)
            return { function: '<anonymous>', file: m[1], line: Number(m[2]) };
        // Python: File "file", line N, in fn
        m = /^File\s+"(.+?)",\s+line\s+(\d+),\s+in\s+(.+)$/.exec(line);
        if (m)
            return { file: m[1], line: Number(m[2]), function: m[3] };
...
function buildRequestPayload(meta) {
    ...
    if (Array.isArray(raw)) {
        for (const t of raw) {
            if (typeof t === 'string')
                ...
                const k = asString(t.key);
                const v = asString(t.value);
                if (k && v !== undefined)
                    pushPair(k, v);
            }
        }
...
//# sourceMappingURL=datadog-parser.js.map