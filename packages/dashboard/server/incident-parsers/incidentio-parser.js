...
// ── Public API ──────────────────────────────────────────────────────────
export function parseIncidentIoEvent(raw) {
    if (!isObject(raw)) {
    ...
    const url = asString(incident.permalink) ??
        asString(incident.url) ??
        asString(incident.reference_url) ??
        '';
    const title = asString(incident.name) ??
    ...
    const occurredAt = toIso(incident.reported_at) ??
...
function pickIncident(raw) {
    if (isObject(raw.data) && isObject(raw.data.incident)) {
        return raw.data.incident;
    }
    if (isObject(raw.incident))
        return raw.incident;
    return raw;
}
function resolveSeverityLabel(incident) {
...
function buildSearchCorpus(incident) {
    ...
    if (Array.isArray(customFields)) {
        for (const entry of customFields) {
            ...
            if (Array.isArray(values)) {
                for (const v of values) {
                    if (!isObject(v))
                        continue;
                    const vv = v;
                    const txt = asString(vv.value_text) ??
                        asString(vv.value_link) ??
                        asString(vv.value);
                    if (txt)
                        parts.push(txt);
                    if (isObject(vv.value_option)) {
                        const opt = asString(vv.value_option.value);
                        if (opt)
                            parts.push(opt);
                    }
                }
            ...
            const flat = asString(entry.value);
            if (flat)
                parts.push(flat);
        }
    }
...
function extractTags(incident) {
    ...
    for (const entry of entries) {
        ...
        if (isObject(e.custom_field)) {
            key =
                asString(e.custom_field.name) ??
                    asString(e.custom_field.key);
        }
        ...
        if (Array.isArray(values)) {
            for (const v of values) {
                if (!isObject(v))
                    continue;
                const vv = v;
                const t = asString(vv.value_text) ??
                    asString(vv.value_link) ??
                    asString(vv.value) ??
...
function resolveAffectedUsers(incident) {
    return (asNumber(incident.affected_users) ??
        asNumber(incident.affectedUsers) ??
        asNumber(incident.impacted_users));
}
//# sourceMappingURL=incidentio-parser.js.map