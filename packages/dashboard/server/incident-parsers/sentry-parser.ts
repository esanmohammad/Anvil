...
export function parseSentryEvent(raw: unknown): ParsedIncident {
  if (!isObject(raw)) {
...
function pickEvent(raw: Record<string, unknown>): unknown {
  // Webhook: { action, data: { event: {...} } } or { event: {...} }
  if (isObject(raw.data) && isObject((raw.data as Record<string, unknown>).event)) {
    return (raw.data as Record<string, unknown>).event;
  }
  if (isObject(raw.event)) return raw.event;
  return raw;
}

function resolveUrl(event: Record<string, unknown>): string {
  ...
  if (groupId) return `https://sentry.io/issues/${groupId}/`;
  return '';
}

function resolveTitle(event: Record<string, unknown>): string {
...
function mapSeverity(level: string | undefined): IncidentSeverity {
...
function extractStack(event: Record<string, unknown>): {
  stackTrace?: string;
  ...
  let chosen: SentryFrame | undefined;
  for (let i = frames.length - 1; i >= 0; i--) {
    const f = frames[i]!;
    if (f.inApp === true) {
      chosen = f;
  ...
  if (!chosen) chosen = frames[frames.length - 1];

  const failingSymbol: FailingSymbol | undefined = chosen
    ? {
...
function findFrames(event: Record<string, unknown>): SentryFrame[] | undefined {
  ...
  for (const f of frames) {
    ...
    out.push({
      ...
      lineNo:
        asNumber(f.lineno) ??
        asNumber(f.lineNo) ??
        asNumber((f as Record<string, unknown>).line_no),
      inApp:
        typeof f.in_app === 'boolean'
          ? f.in_app
...
function extractRequest(event: Record<string, unknown>): unknown {
  ...
  if (queryString !== undefined) out.queryString = queryString;
  if (cookies !== undefined) out.cookies = cookies;
  return Object.keys(out).length > 0 ? out : undefined;
}

function extractTags(event: Record<string, unknown>): {
  ...
  for (const t of raw) {
    ...
    if (Array.isArray(t) && t.length >= 2) {
      ...
      key = asString((t as Record<string, unknown>).key);
      value = asString((t as Record<string, unknown>).value);
    }
...
function resolveAffectedUsers(event: Record<string, unknown>): number | undefined {
  const direct = asNumber(event.userCount) ?? asNumber(event.user_count);
  if (direct !== undefined) return direct;
  if (isObject(event.count)) {
    const c = asNumber((event.count as Record<string, unknown>).users);
