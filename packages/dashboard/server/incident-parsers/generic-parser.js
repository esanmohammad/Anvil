...
import { asString, findGithubPrUrl, } from './types.js';
// ── Public API ──────────────────────────────────────────────────────────
export function parseGenericStackTrace(input) {
    ...
    const linkedPrUrl = findGithubPrUrl([input.title, input.summary, input.url, stackTrace].filter(Boolean).join('\n'));
    return {
        externalId,
        source: 'manual',
        url,
 ...
 */
export function extractFailingSymbol(stackTrace) {
 ...
 */
function parseFrames(stackTrace) {
...
function deriveTitle(stackTrace, symbol) {
    ...
    if (symbol)
        return `${symbol.function} (${symbol.file}:${symbol.line})`;
    return undefined;
}
function hashStack(stackTrace) {
