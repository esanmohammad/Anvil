...
export async function tryWebhookRoutes(req, res, anvilHome, kbManagerRef, webhookDepsRef) {
            ...
            const handled = await dispatchIncidentWebhook(req, res, {
                ...
                resolveProject: (u) => {
                    ...
                    if (Array.isArray(h) && h[0])
                        return h[0].trim() || null;
                    return null;
                },
                onIncident: async (parsed, autoReplay) => {
                    try {
                        const project = req.headers['x-anvil-project']
    ...
    if (url.pathname === '/api/pipeline/approve' && webhookDepsRef?.current) {
        ...
        try {
            if (payload.action === 'approve') {
                ...
                deps.services.pipeline.emit('pipeline.resumed', { pause: pauseStore.get(payload.runId) });
            }
                ...
                deps.services.pipeline.emit('pipeline.cancelled', { pause: pauseStore.get(payload.runId) });
            }
        }
        catch (err) {
    ...
    if (url.pathname === '/api/contracts/drift' && req.method === 'POST' && webhookDepsRef?.current) {
        ...
        try {
            ...
            const graph = build(contracts, calls);
            res.writeHead(200, { 'Content-Type': 'application/json' });
    ...
    if (url.pathname === '/api/tests/rank' && req.method === 'POST' && webhookDepsRef?.current) {
        ...
        for await (const chunk of req)
            body += chunk;
        try {
            const { project, changedSymbols } = JSON.parse(body || '{}');
            ...
            const result = rank({ changedSymbols: changedSymbols, repoGraphs });
            res.writeHead(200, { 'Content-Type': 'application/json' });
    ...
    if (url.pathname === '/api/triage/analyze' && req.method === 'POST' && webhookDepsRef?.current) {
        ...
        for await (const chunk of req)
            body += chunk;
        try {
            const { logText, logSource, project } = JSON.parse(body || '{}');
            if (!logText) {
                res.writeHead(400, { 'Content-Type': 'application/json' });
