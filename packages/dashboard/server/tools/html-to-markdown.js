...
export function htmlToMarkdown(html) {
    ...
    // 5. Anchors.
    s = s.replace(/<a\b[^>]*href\s*=\s*"([^"]*)"[^>]*>([\s\S]*?)<\/a\s*>/gi, (_m, href, body) => `[${stripTags(body).trim()}](${href})`);
    s = s.replace(/<a\b[^>]*>([\s\S]*?)<\/a\s*>/gi, (_m, body) => stripTags(body).trim());
    // 6. Code/pre.
    s = s.replace(/<pre\b[^>]*>([\s\S]*?)<\/pre\s*>/gi, (_m, body) => '\n\n```\n' + decodeEntities(stripTags(body)) + '\n```\n\n');
    s = s.replace(/<code\b[^>]*>([\s\S]*?)<\/code\s*>/gi, (_m, body) => '`' + decodeEntities(stripTags(body)) + '`');
    // 7. Bold / italic.
    s = s.replace(/<(?:strong|b)\b[^>]*>([\s\S]*?)<\/(?:strong|b)\s*>/gi, (_m, body) => `**${stripTags(body)}**`);
    s = s.replace(/<(?:em|i)\b[^>]*>([\s\S]*?)<\/(?:em|i)\s*>/gi, (_m, body) => `*${stripTags(body)}*`);
    // 8. Lists.
    s = s.replace(/<li\b[^>]*>([\s\S]*?)<\/li\s*>/gi, (_m, body) => `\n- ${stripTags(body).trim()}`);
...
export function looksLikeSpaShell(markdown) {
    const text = markdown.replace(/\s+/g, ' ').trim();
    if (text.length < SSR_LOWER_BOUND)
        return true;
