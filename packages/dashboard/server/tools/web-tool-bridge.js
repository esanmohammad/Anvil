...
import { BrowserSessionRegistry } from '../browser/session-manager.js';
 ...
 */
export function createWebToolBridge(opts = {}) {
    let cachedSearch;
    let cachedFetch;
    return {
        search: {
...
function enrichCtx(ctx) {
    if (ctx.runId && ctx.sessionId && ctx.projectSlug)
        return ctx;
...
function createComputerBackend(opts) {
    ...
    return {
        async do(action, ctx) {
            ...
            return runner.do(action);
        },
    };
}
function createBrowserBackend(opts) {
    ...
    const screenshotLimits = new Map();
    function sessionKey(ctx) {
        return `${ctx.runId ?? 'standalone'}\u241F${ctx.sessionId ?? ''}`;
    }
    function detectorFor(ctx) {
        const k = sessionKey(ctx);
        let d = detectors.get(k);
        if (!d) {
            d = new NoProgressDetector();
            detectors.set(k, d);
        }
        return d;
    }
    function clickLimitFor(ctx) {
        const k = sessionKey(ctx);
        let r = clickLimits.get(k);
        if (!r) {
            r = new RateLimiter(1, 1000);
            clickLimits.set(k, r);
        }
        return r;
    }
    function screenshotLimitFor(ctx) {
        const k = sessionKey(ctx);
        let r = screenshotLimits.get(k);
        if (!r) {
            r = new RateLimiter(6, 60_000);
            screenshotLimits.set(k, r);
        }
        return r;
    }
    function recordProgress(ctx, kind, state) {
        const tuple = {
            url: state.url,
    ...
    function clearSessionState(ctx) {
        const k = sessionKey(ctx);
        detectors.delete(k);
    ...
    return {
        ...
        async consoleMessages(args, ctx) {
            const session = acquire(ctx);
            const runner = await session.getRunner();
            return runner.consoleMessages(args);
        },
        async networkRequests(args, ctx) {
            const session = acquire(ctx);
            const runner = await session.getRunner();
            return runner.networkRequests(args);
        },
        async newTab(args, ctx) {
            const session = acquire(ctx);
            const runner = await session.getRunner();
...
//# sourceMappingURL=web-tool-bridge.js.map