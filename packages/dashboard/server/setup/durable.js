...
export async function bootDurable(deps) {
    const takenOverRunIds = [];
    // Phase D3 + F4 — migration + orphan takeover.
    try {
        const store = getDurableStore();
        const migrationStats = await runDurableMigration(store, {
            onTakeover: (runIds) => {
                console.log(`[dashboard] auto-takeover: claimed ${runIds.length} orphaned run(s) — ${runIds.join(', ')}`);
                takenOverRunIds.push(...runIds);
            },
        });
        if (migrationStats.scanned > 0 || migrationStats.orphaned > 0 || migrationStats.takenOver > 0) {
            console.log(`[dashboard] durable migration: scanned=${migrationStats.scanned} migrated=${migrationStats.migrated} orphaned=${migrationStats.orphaned} takenOver=${migrationStats.takenOver} contested=${migrationStats.takeoverContested} errors=${migrationStats.errors}`);
        }
    }
    catch (err) {
    ...
    if (takenOverRunIds.length > 0) {
        ...
        if (autoResumeEnabled) {
            void dispatchTakenOverRuns(getDurableStore(), takenOverRunIds, (project, feature, options) => {
                deps.startPipeline(project, feature, options);
            }, deps.stagesByName).then((stats) => {
                if (stats.dispatched > 0 || stats.errors > 0) {
                    console.log(`[dashboard] auto-resume: attempted=${stats.attempted} dispatched=${stats.dispatched} skipped=${stats.skipped} errors=${stats.errors}`);
                }
            });
        }
        else {
            ...
            const store = getDurableStore();
            if (store) {
                void Promise.all(takenOverRunIds.map((runId) => store.updateRunStatus(runId, 'paused').catch(() => { }))).then(() => {
                    console.log(`[dashboard] auto-resume disabled — ${takenOverRunIds.length} orphan(s) marked paused (use Replay button to resume manually)`);
                });
            }
        }
    }
    // Phase F3 — vacuum schedule.
    let vacuumHandle = null;
    try {
        const store = getDurableStore();
