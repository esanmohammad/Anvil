...
import { WS_OPEN, type WsClient } from './ws-client.js';
...
export function listenAndReturnHandle(deps: ListenDeps): Promise<DashboardServerHandle> {
  return new Promise<DashboardServerHandle>((resolve) => {
    deps.server.listen(deps.port, () => {
      ...
      if (process.env.ANVIL_DISABLE_SOCKET_IO !== '1') {
        ...
        const fauxWsForSocket = (socket: import('socket.io').Socket): WsClient => ({
          readyState: WS_OPEN,
          send: (raw: string) => {
            try {
        ...
        const socketHandle = mountSocketServer({
          ...
          onAction: async (socket, msg) => {
            if (msg && typeof msg === 'object') {
              await deps.handleClientMessage(fauxWsForSocket(socket), msg as ClientMessage);
            }
          },
        });
      ...
      const stop = async (): Promise<void> => {
        ...
        for (let i = 0; i < deps.stopHandlers.length; i++) {
          ...
          if (process.env.ANVIL_WS_DIAG) console.warn('[stop-diag] handler', i, 'start');
          try {
            await Promise.race([
              Promise.resolve(fn()),
          ...
          if (process.env.ANVIL_WS_DIAG) console.warn('[stop-diag] handler', i, 'done');
        }
        if (process.env.ANVIL_WS_DIAG) console.warn('[stop-diag] closing connections');
        try {
          (deps.server as { closeAllConnections?: () => void }).closeAllConnections?.();
        } catch { /* older Node */ }
        if (process.env.ANVIL_WS_DIAG) console.warn('[stop-diag] server.close');
        await Promise.race([
          new Promise<void>((r) => deps.server.close(() => r())),
          new Promise<void>((r) => setTimeout(r, 2000).unref()),
        ]);
        if (process.env.ANVIL_WS_DIAG) console.warn('[stop-diag] done');
      };
