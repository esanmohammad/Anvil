...
export function createInitSender(deps: InitSenderDeps): SendInitFn {
  ...
  void deps.discoverAvailableModels().catch(() => { /* best-effort warm */ });
  ...
  return async function sendInit(ws: WsClient): Promise<void> {
    try {
      ...
      const fallbackModels = (): AvailableModelsLike =>
        ({ providers: [], defaultModel: 'sonnet', defaultProvider: 'claude' });
      ...
      const modelsBounded = Promise.race([
        ...
        new Promise<AvailableModelsLike>((resolve) => {
