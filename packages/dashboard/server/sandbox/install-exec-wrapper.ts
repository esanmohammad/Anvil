...
const hashers = new Map<string, () => Promise<string>>();

export function installSandboxExecWrapper(): void {
  setSandboxExecWrapper(async ({ ctx, handle, execArgs, idx, fn }) => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const c = ctx as any;
    const runId = (c?.runId as string | undefined) ?? 'unknown';
    const stage = (c?.stage as string | undefined) ?? 'unknown';
    const handleKey = `${runId}:${stage}:${handle?.id ?? 'unknown'}`;
    let hasher = hashers.get(handleKey);
    if (!hasher) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      hasher = buildHandleStateHasher(handle as any);
      hashers.set(handleKey, hasher);
    }
    return wrapSandboxExec(
      ctx,
      {
        runId,
        stage,
...
export function __clearSandboxExecHashersForTests(): void {
  hashers.clear();
}
