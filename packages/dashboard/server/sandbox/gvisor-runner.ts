...
export class GVisorSandboxRunner extends DockerSandboxRunner {
  constructor(opts: DockerSandboxOptions = {}) {
    ...
    this.dockerCli = (argv: readonly string[]) => {
  ...
  override async isAvailable(): Promise<boolean> {
    ...
    const probe = await new Promise<boolean>((resolve) => {
      const spawnFn = (this as unknown as { opts: { spawnFn?: typeof spawn } }).opts.spawnFn ?? spawn;
      const child = spawnFn('runsc', ['--version'], { stdio: ['ignore', 'pipe', 'pipe'] });
      let exited = false;
      child.on('error', () => { if (!exited) { exited = true; resolve(false); } });
      child.on('exit', (code) => { if (!exited) { exited = true; resolve(code === 0); } });
    });
    return probe;
  }
}
