 ...
 */
export function buildCacheMounts(opts: CacheMountOptions = {}): CacheMount[] {
  const home = opts.homeDir ?? os.homedir();
  const defaultMode: CacheMode = opts.defaultMode ?? 'read-only';
  const out: CacheMount[] = [];
  for (const [tool, def] of Object.entries(CACHE_DEFINITIONS) as [keyof typeof CACHE_DEFINITIONS, { host: string; sandbox: string }][]) {
 ...
 */
export function dockerCacheMountArgs(mounts: readonly CacheMount[]): string[] {
  const args: string[] = [];
