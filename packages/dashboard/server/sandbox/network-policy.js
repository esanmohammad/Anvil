 ...
 */
export function resolveNetworkPolicy(args) {
    ...
    if (project?.blockList)
        blockList.push(...project.blockList);
    if (stage?.blockList)
        for (const b of stage.blockList)
            if (!blockList.includes(b))
                blockList.push(b);
    const allowList = [];
    const sources = [];
    const addAll = (xs, src) => {
        ...
        if (added && !sources.includes(src))
            sources.push(src);
    };
    addAll(stage?.allowList, 'stage');
    addAll(project?.allowList, 'project');
    if (includePm)
        addAll(PACKAGE_MANAGER_ALLOW_LIST, 'package-manager');
    const out = {
    ...
    if (project?.dnsResolver !== undefined)
        out.dnsResolver = project.dnsResolver;
    else if (stage?.dnsResolver !== undefined)
        out.dnsResolver = stage.dnsResolver;
    return out;
}
/**
 ...
 */
export function dockerRunNetworkArgs(policy, opts = {}) {
 ...
 */
export function dnsmasqConfigBody(policy) {
    const lines = [
    ...
    for (const entry of policy.allowList ?? []) {
        if (entry === 'localhost' || entry === '127.0.0.1' || entry === '::1')
            continue;
 ...
 */
export function iptablesRulesForPolicy(policy) {
    const rules = [];
