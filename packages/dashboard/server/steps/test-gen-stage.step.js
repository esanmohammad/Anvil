 ...
 */
export function pickRepoForBehavior(behavior, repoLocalPaths) {
    ...
    for (const [repoName, path] of Object.entries(repoLocalPaths)) {
        if (!path || !existsSync(path))
            continue;
        try {
            const found = execSync(`find "${path}" -name "${targetBase}" -not -path "*/node_modules/*" | head -1`, { encoding: 'utf-8', timeout: 5_000 }).trim();
            if (found)
                return repoName;
        }
        catch { /* continue */ }
...
export async function runTestGenForProject(opts) {
    ...
    const conventions = await fingerprintConventions(Object.values(opts.repoLocalPaths).find((p) => existsSync(p)) ?? opts.workspaceDir);
    ...
    const spec = specStore.createSpec(opts.project, plan.title || plan.slug, plan.model ?? opts.model, {
