...
export interface ShellRunner {
  run: (cmd: string, opts: { cwd: string; timeout?: number }) => string;
}
 ...
 */
export async function pullBaseBranchForRepos(
  opts: PullBaseBranchOptions,
): Promise<void> {
  ...
  const pullBranch = (cwd: string, label: string): boolean => {
    if (opts.baseBranch) {
      try {
        runner.run(
          `git fetch origin && git checkout "${opts.baseBranch}" && git pull origin "${opts.baseBranch}"`,
          { cwd, timeout: 30_000 },
        );
 ...
 */
export function runPostBuildGuards(opts: RunPostBuildGuardsOptions): void {
  const runner = opts.runner ?? defaultRunner;
  ...
  for (const repo of opts.repos) {
    try {
      ...
      if (!repoCommands?.format && !repoCommands?.lint) {
        ...
        const hasPython =
          fileExists(repo.path, 'pyproject.toml') || fileExists(repo.path, 'setup.py');
 ...
 */
export function deployProject(opts: DeployProjectOptions): void {
  if (!opts.mode) return;
 ...
 */
export function createFeatureBranches(opts: CreateFeatureBranchesOptions): void {
  const branchName = `anvil/${opts.featureSlug}`;
  const runner = opts.runner ?? defaultRunner;

  const ensureBranch = (cwd: string, label: string): void => {
    try {
      try {
