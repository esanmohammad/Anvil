...
export interface CanonicalComputerToolSpec {
  display: { width_px: number; height_px: number; allowZoom?: boolean };
}

export type ProviderToolSchema =
  | { provider: 'anthropic'; schema: { type: 'computer_20251124'; name: 'computer'; display_width_px: number; display_height_px: number } }
  | { provider: 'openai'; schema: { type: 'computer_use_preview'; display_width: number; display_height: number; environment: 'browser' } }
  | { provider: 'gemini'; schema: { name: 'computer'; description: string; parameters: object } }
  | { provider: 'unsupported'; schema: null };
 ...
 */
export function detectProvider(model: string): CanonicalProvider {
  const m = model.toLowerCase();
  if (m.includes('claude') || m.startsWith('opus') || m.startsWith('sonnet') || m.startsWith('haiku')) return 'anthropic';
  if (m.includes('gpt') || m.includes('o1-') || m.includes('o3-') || m.includes('cua')) return 'openai';
  if (m.includes('gemini')) return 'gemini';
  if (m.startsWith('ollama/') || m.includes('llama') || m.includes('mistral')) return 'ollama';
  return 'unsupported';
}
 ...
 */
export function hasPixelBrowser(model: string): boolean {
  const provider = detectProvider(model);
  if (provider === 'unsupported' || provider === 'ollama') return false;
...
export function translateComputerTool(
  spec: CanonicalComputerToolSpec,
  model: string,
): ProviderToolSchema {
  const provider = detectProvider(model);
  switch (provider) {
 ...
 */
export function translateActionToProvider(
...
function translateForAnthropic(action: ComputerAction): unknown {
...
function translateForOpenAI(action: ComputerAction): unknown {
  switch (action.action) {
    case 'click':
