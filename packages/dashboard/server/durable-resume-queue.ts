...
export async function dispatchTakenOverRuns(
  ...
  // Off by default — auto-resuming on every dashboard boot is surprising
  // and ties the user's hands. Opt-in via ANVIL_DURABLE_AUTO_RESUME=1.
  // The takeover scan still claims orphan leases (so manual resume works);
  // only the auto-dispatch is gated.
  const disabled = opts.disabled ?? process.env.ANVIL_DURABLE_AUTO_RESUME !== '1';
