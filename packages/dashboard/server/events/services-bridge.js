...
function buildEnvelope(kind, payload, now) {
    const seed = { kind, payload };
    const topics = roomsForEvent(seed);
    return {
 ...
 */
export function bridgeServicesToRooms(opts) {
    const { services, io, replay } = opts;
    const now = opts.now ?? Date.now;
    const unsubs = [];
    function bind(svc) {
        const off = svc.onAny((kind, payload) => {
            let ev;
            try {
            ...
            try {
                ...
                if (legacy)
                    io.to(ev.topics).emit(legacy.type, legacy.payload);
            }
            catch (err) {
