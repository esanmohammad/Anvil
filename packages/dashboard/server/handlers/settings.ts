...
export function settingsRoutes(): Record<string, Handler> {
  return {
    'get-providers': route({
    ...
    'get-available-models': route({
      ...
      handle: async (_input, deps) => {
        if (!deps.extras.discoverAvailableModels) return;
    ...
    'get-budget-status': route({
      ...
      handle: async (input, deps) => {
        ...
        const loader = deps.extras.projectLoader as
          | { getBudgetConfig(p: string): { max_per_run?: number; max_per_day?: number; alert_at?: number };
              getModelForStage?(p: string, s: string): unknown }
          | undefined;
        if (!loader) return;
        let budgetConfig: { max_per_run?: number; max_per_day?: number; alert_at?: number } = {};
        try { budgetConfig = loader.getBudgetConfig(project); }
        catch { return { maxPerRun: 100, maxPerDay: 200, alertAt: 80, todaySpent: 0 }; }
        let todaySpent = 0;
        try {
    ...
    'list-memories': route({
      ...
      handle: async (input, deps) => {
        ...
        try {
          ...
          const inspector = new MemoryInspector(store.unwrap() as ConstructorParameters<typeof MemoryInspector>[0]);
          const project = input.project ?? '';
          const filter = {
            namespace: project ? { scope: 'project' as const, projectId: project } : undefined,
            search: input.search,
            kind: input.kind as undefined,
            limit: input.limit,
          };
    ...
    'memory-add': route({
      ...
      handle: (input, deps) => {
        ...
        deps.extras.buildProjectOverview?.(project).then((o) =>
          deps.ws.send(JSON.stringify({ type: 'overview', payload: o })),
        ).catch(() => { /* non-fatal */ });
      },
    }),
    ...
    'generate-conventions': route({
      ...
      handle: async (input, deps) => {
        ...
        if (!paths || !loader || !getWorkspace) return { rules: [] };
        try {
          const { join } = await import('node:path');
    ...
    'set-budget': route({
      ...
      handle: (input, deps) => {
        ...
        if (!project) return;
        const loader = deps.extras.projectLoader;
        if (!loader?.saveBudgetConfig) return;
        try {
