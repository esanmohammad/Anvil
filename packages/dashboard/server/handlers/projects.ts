...
export function projectRoutes(): Record<string, Handler> {
  return {
    /**
    ...
    'rollback-run': route({
      ...
      handle: async (input, deps) => {
        const allRuns = (deps.extras.loadRunsSync?.() ?? []) as Array<{
          id: string; project: string; featureSlug?: string;
        }>;
    ...
    'kill-agent': route({
      ...
      onParseFail: 'silent',
      handle: (input, deps) => {
        if (input.agentId) deps.extras.killAgent?.(input.agentId);
      },
    }),
    ...
    'send-input': route({
      ...
      onParseFail: 'silent',
      handle: (input, deps) => {
        if (input.text) deps.extras.sendInput?.(input.text, input.agentId);
      },
    }),
    ...
    'get-run': route({
      ...
      handle: async (input, deps) => {
        ...
        const allRuns = (deps.extras.loadRunsSync?.() ?? []) as Array<{
          id: string; runType?: string; project: string; feature: string; model: string;
          status: string; startedAt: number; totalCost?: unknown; durationMs?: number;
          stageDetails?: unknown; prUrls?: unknown[]; output?: unknown;
        }>;
        ...
        try {
          ...
          const features = (deps.extras.featureStore?.listFeatures() ?? []) as Array<{ project: string; slug: string }>;
    ...
    'get-overview': route({
      ...
      onParseFail: 'silent',
      handle: async (input, deps) => {
        if (!deps.extras.buildProjectOverview) return;
