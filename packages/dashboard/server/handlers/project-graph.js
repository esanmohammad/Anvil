...
export function projectGraphRoutes() {
    return {
            ...
            handle: async (input, deps) => {
                ...
                try {
                    ...
                    if (existsSync(projectGraphPath)) {
                        try {
                            projectGraph = JSON.parse(readFileSync(projectGraphPath, 'utf-8'));
                        }
                        catch { /* ok */ }
                    }
                    const repoStats = [];
                        ...
                        try {
                            ...
                            if (crossRepoEdges.length > 0) {
                                if (!projectGraph)
                                    ...
                                    projectGraph.relationships = [...(projectGraph.relationships ?? []), ...crossRepoEdges];
                            }
                        }
                        catch { /* non-fatal */ }
                    }
