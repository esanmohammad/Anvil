        ...
        'get-plan-lineage': route({
            ...
            handle: (input, deps) => {
                ...
                const lineage = versions.map((n) => {
                    const p = store.readVersion(input.project, input.planSlug, n);
                    if (!p)
                        return { version: n, updatedAt: '', createdBy: undefined };
                    return {
                        version: p.version,
        ...
        'save-plan': route({
            ...
            handle: async (input, deps) => {
                try {
                    ...
                    const editedSections = Object.keys(input.plan).join(', ') || 'unknown';
...
// ── helpers ─────────────────────────────────────────────────────────────
function safeBudget(loader, project) {
    if (!loader)
        return {};
    try {
        return loader.getBudgetConfig(project);
    }
    catch {
        return {};
    }
}
function safeRepoMap(loader, project) {
    if (!loader)
        return {};
    try {
