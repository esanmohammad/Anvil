...
export function testRoutes(): Record<string, Handler> {
  return {
    ...
    'set-coverage-sla': route({
      ...
      handle: async (input, deps) => {
        ...
        writeProjectSLA(deps.extras.anvilHome, input.project, input.sla as never);
    ...
    'share-test-spec': route({
      ...
      handle: async (input, deps) => {
        const store = deps.extras.testSpecStore;
        if (!store) return;
        const spec = store.readCurrent(input.project, input.slug) as
          | { project: string; slug: string; version: number }
          | null;
    ...
    'check-coverage-sla': route({
      ...
      handle: async (input, deps) => {
        ...
        const all = runStore.listRuns(input.project, input.slug) as Array<{ id: string; completedAt?: string }>;
        const prev = all.find((r) => r.id !== input.runId && r.completedAt) ?? null;
        const report = checkCoverageSLA(
          run as Parameters<typeof checkCoverageSLA>[0],
          prev as Parameters<typeof checkCoverageSLA>[1],
          sla,
        );
        return { report };
      },
    ...
    'plan-parallelization': route({
      ...
      handle: async (input, deps) => {
        ...
        const runStore = deps.extras.testRunStore;
        if (!specStore || !caseStore || !runStore) return;
        const spec = specStore.readCurrent(input.project, input.slug) as
          | { version: number; conventions?: { runner?: string } }
          | null;
        if (!spec) return { error: 'spec-not-found' };
        ...
        const plan = planParallelization(
          runs as Parameters<typeof planParallelization>[0],
          cases as Parameters<typeof planParallelization>[1],
          { targetShardDurationMs: 60_000, maxShards: 8, minShards: 1 },
        );
        const matrix = emitCIMatrix(plan, (input.runner ?? spec.conventions?.runner) as Parameters<typeof emitCIMatrix>[1]);
        return { plan, matrix };
      },
    ...
    'detect-stale-tests': route({
      ...
      handle: async (input, deps) => {
        ...
        if (!specStore || !caseStore || !runStore || !loader) return;
        const spec = specStore.readCurrent(input.project, input.slug) as { version: number } | null;
        if (!spec) return { error: 'spec-not-found' };
        ...
        const candidates = await detectStaleTests(
          runs as Parameters<typeof detectStaleTests>[0],
          cases as Parameters<typeof detectStaleTests>[1],
          { repoLocalPath: repoPath, runsWindow: 20, minNonFailRuns: 15 },
        );
        return { candidates };
      },
    ...
    'publish-test-checks': route({
      ...
      handle: async (input, deps) => {
        const specStore = deps.extras.testSpecStore;
        const runStore = deps.extras.testRunStore;
        if (!specStore || !runStore) return;
        const spec = specStore.readCurrent(input.project, input.slug);
        const run = runStore.readRun(input.project, input.slug, input.runId);
        if (!spec || !run) return { error: 'spec-or-run-missing' };
        ...
        const result = await publishTestChecks({
          ...
          spec: spec as Parameters<typeof publishTestChecks>[0]['spec'],
          run: run as Parameters<typeof publishTestChecks>[0]['run'],
          minSeverity: 'info',
        });
        return result;
      },
    ...
    'rank-tests-for-pr': route({
      ...
      handle: async (input, deps) => {
        ...
        const result = rankRelevantTests({
          changedSymbols: input.changedSymbols as Parameters<typeof rankRelevantTests>[0]['changedSymbols'],
          repoGraphs,
        });
