    ...
    return {
        ...
        'list-cost-breaches': route({
            ...
            handle: async (input, deps) => {
                ...
                const out = [];
                if (existsSync(dir)) {
                    const projects = input.project ? [input.project] : readdirSync(dir).filter((n) => !n.includes('.'));
        ...
        'subscribe-cost': route({
            ...
            handle: (input, deps) => {
                try {
                    deps.extras.broadcastCostSnapshot?.(input.project, input.runId);
                }
                catch { /* ok */ }
            },
        }),
        ...
        'update-pipeline-policy': route({
            ...
            handle: async (input, deps) => {
                ...
                try {
                    deps.extras.broadcastCostSnapshot?.(input.project);
                }
                catch { /* ok */ }
