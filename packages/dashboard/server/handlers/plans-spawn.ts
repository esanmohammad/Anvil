...
export function plansSpawnRoutes(): Record<string, Handler> {
  return {
    'run-plan': route({
      input: Z.RunPlan,
      onParseFail: 'silent',
    ...
    'run-plan-variants': route({
      ...
      handle: (input, deps) => {
        ...
        actions.spawnPlanVariants(project, feature, variants as unknown[], options?.model);
      },
    }),
    ...
    'auto-refine-plan': route({
      ...
      handle: async (input, deps) => {
        ...
        if (!planStore || !validator || !dispatch || !refine) return;
    ...
    'execute-plan': route({
      ...
      handle: async (input, deps) => {
        ...
        const validator = deps.extras.planValidator;
        const dispatch = deps.extras.dispatchLifecycle;
        if (!actions || !planStore || !validator || !dispatch) return;
        ...
        actions.startPipeline(project, shortFeature, {
          ...
          modelTier: (options as { modelTier?: string } | undefined)?.modelTier,
