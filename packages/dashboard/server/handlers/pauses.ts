...
export function pauseRoutes(): Record<string, Handler> {
  return {
    /**
    ...
    'cancel-pipeline-pause': route({
      ...
      handle: async (input, deps) => {
        ...
        const env = handleCancelPause(
          store as unknown as Parameters<typeof handleCancelPause>[0],
          input as unknown as Record<string, unknown>,
          deps.user ?? deps.extras.defaultUser,
        );
        deps.ws.send(JSON.stringify(env));
        const state = store.get(input.runId ?? '');
        if (state) deps.services.pipeline.emit('pipeline.cancelled', { pause: state } as never);
      },
    }),
    ...
    'list-pipeline-pauses': route({
      ...
      handle: async (input, deps) => {
        ...
        const env = handleListPauses(
          store as unknown as Parameters<typeof handleListPauses>[0],
          input as unknown as Record<string, unknown>,
        );
        deps.ws.send(JSON.stringify(env));
      },
    ...
    'get-pipeline-pause': route({
      ...
      handle: async (input, deps) => {
        ...
        const env = handleGetPause(
          store as unknown as Parameters<typeof handleGetPause>[0],
          input as unknown as Record<string, unknown>,
        );
        deps.ws.send(JSON.stringify(env));
      },
