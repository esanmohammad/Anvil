...
export function testsPipelineRoutes() {
    return {
        ...
        'create-test-spec-from-plan': route({
            ...
            handle: async (input, deps) => {
                ...
                try {
                    ...
                    const cases = resolvedBehaviors.map((b) => emitTestCase(b, conventions, {
                        specSlug: spec.slug,
            ...
            handle: async (input, deps) => {
                ...
                try {
                    ...
                    const exec = await executeTestRun({
                        ...
                        onLog: (stream, line) => {
                            deps.services.tests.emit('test.run-log', { runId: run.id, stream, line });
                        },
                    });
                    ...
                    for (const caseId of exec.flakyQuarantined) {
                        ...
                        if (r?.flakyScore != null) {
                            try {
                                stores.testLearningsStore.recordFlaky(project, caseId, r.flakyScore);
                            }
                            catch { /* ok */ }
                        }
                    }
        ...
        'review-test-spec': route({
            ...
            handle: async (input, deps) => {
                ...
                if (!stores || !projectLoader || !agentManager)
                    return;
                ...
                try {
                    ...
                    const result = await runMultiPersonaReview({
                        ...
                        onPersonaStart: (persona, agentId) => {
                            deps.services.tests.emit('test.review-persona-start', { runId, persona, agentId });
                        },
                        onPersonaDone: (persona, findings, cost) => {
                            deps.services.tests.emit('test.review-persona-done', { runId, persona, findingCount: findings.length, cost });
                        },
                        onError: (persona, message) => {
                            deps.services.tests.emit('test.review-persona-error', { runId, persona, message });
                        },
                    });
                    ...
                    deps.services.tests.emit('test.review-complete', {
                        ...
                        perPersona: Object.fromEntries(Object.entries(result.perPersonaFindings).map(([k, v]) => [k, v.length])),
                    });
                }
                catch (err) {
                    deps.ws.send(JSON.stringify({
            ...
            handle: async (input, deps) => {
                ...
                try {
                    ...
                    const result = await runMutationTesting({
                        ...
                        timeoutMs: 600_000,
                        onLog: (stream, line) => {
                            deps.services.tests.emit('test.mutation-log', { runId, stream, line });
                        },
                    });
                    if (result.supported && result.score != null) {
                        ...
                        try {
                            stores.testLearningsStore.updateMutationScore(project, result.byFile);
                        }
                        catch { /* ok */ }
                    }
                    const updated = stores.testRunStore.readRun(project, slug, runId);
                ...
                try {
                    ...
                    const result = await runTestAuthor({
                        ...
                        onCaseStart: (caseId, agentId) => {
                            deps.services.tests.emit('test.polish-case-start', { slug, caseId, agentId });
                        },
                        onCaseDone: (caseId, updated, cost) => {
                            deps.services.tests.emit('test.polish-case-done', { slug, caseId, cost, case: updated });
                        },
                        onError: (caseId, message) => {
                            deps.services.tests.emit('test.polish-case-error', { slug, caseId, message });
                        },
                    });
                    deps.ws.send(JSON.stringify({
        ...
        'resolve-test-finding': route({
            ...
            handle: (input, deps) => {
                ...
                deps.services.tests.emit('test.finding-resolved', { runId, findingId, resolution, run: updated });
            },
        }),
                ...
                try {
                    ...
                    if (slug) {
                        ...
                        if (current) {
                            ...
                            const newCases = result.behaviors.map((b) => emitTestCase(b, current.conventions, { specSlug: slug, specVersion: next.version, projectSlug: project }));
            ...
            handle: async (input, deps) => {
                ...
                try {
                    ...
                    const result = await analyzeFlakiness({
                        ...
                        cwd: repoPath,
                        model,
                        onAnalyzeStart: (caseId, agentId) => deps.services.tests.emit('test.flakiness-case-start', { runId, caseId, agentId }),
                        onAnalyzeDone: (caseId, finding) => deps.services.tests.emit('test.flakiness-case-done', { runId, caseId, finding }),
                        onError: (caseId, message) => deps.services.tests.emit('test.flakiness-case-error', { runId, caseId, message }),
                    });
...
//# sourceMappingURL=tests-pipeline.js.map