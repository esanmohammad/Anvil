...
export interface HandlerExtras {
    ...
    dispatchLifecycle?: (project: string, planSlug: string, event: unknown) => Promise<unknown>;
...
export interface PipelineActionsBundle {
    ...
    startReviewRun: (project: string, prUrl: string, trigger: string, personas: string[], model?: string, prior?: unknown) => Promise<void>;
...
interface ProjectLoaderShape {
    getBudgetConfig(project: string): {
        max_per_run?: number;
        max_per_day?: number;
        alert_at?: number;
    };
    saveBudgetConfig?(project: string, cfg: {
        max_per_run?: number;
        max_per_day?: number;
        alert_at?: number;
    }): void;
...
interface PlanStoreShape {
    ...
    readPointer(project: string, planSlug: string): {
 ...
 */
type HandleReturn<O> = void | undefined | O | {
    error: string;
};
/**
 ...
 */
export declare function route<I, O>(opts: {
    input: ZodType<I>;
