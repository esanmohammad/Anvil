...
export function incidentsSpawnRoutes() {
    return {
            ...
            handle: async (input, deps) => {
                ...
                try {
                    ...
                    const result = await runReplayPipeline({
                        ...
                        repoLocalPaths,
                        onStep: (step, state) => {
                            deps.services.incidents.emit('replay.step', { incidentId, step, state });
                        },
                    });
                    ...
                    if (result.attempt.confidence === 'low'
                        ...
                        try {
                            ...
                            if (incident)
                                await notifyLowConfidenceReplay(incident, result.attempt);
                        }
                        catch { /* non-fatal */ }
                    }
...
//# sourceMappingURL=incidents-spawn.js.map