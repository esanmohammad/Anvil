...
const GENERIC_SECRET_RE =
  /\b(password|passwd|api[_-]?key|secret|token)\s*[:=]\s*['"]([^'"\s]{7,})['"]/i;
...
const OPEN_REDIRECT_RE =
  /\bres\.redirect\s*\(\s*req\.(?:query|params|body|headers)\b/;

// 5) CORS misconfig (OWASP A05:2021 — Security Misconfiguration)
const CORS_WILDCARD_RE =
  /Access-Control-Allow-Origin\s*['"]?\s*[:,]\s*['"]\*['"]|['"]Access-Control-Allow-Origin['"]\s*,\s*['"]\*['"]/;
...
interface BuildFindingArgs {
  ...
  description: string;
  suggestedFix?: ReviewFinding['suggestedFix'];
}

function build(args: BuildFindingArgs): ReviewFinding {
  ...
  if (args.suggestedFix) f.suggestedFix = args.suggestedFix;
  return f;
}

// ── Main entry point ────────────────────────────────────────────────────

export function runSecurityPrepass(diff: DiffInput): ReviewFinding[] {
  ...
  for (const hit of matchInAddedLines(diff, AWS_ACCESS_KEY_RE)) {
    findings.push(build({
      ...
      confidence: 'high',
      description:
        'Hardcoded AWS access key ID. Rotate the key immediately, revoke it in IAM, and use IAM roles or a secret manager instead of embedding credentials.',
  ...
  for (const hit of matchInAddedLines(diff, AWS_SECRET_KEY_RE)) {
    findings.push(build({
      ...
      description:
        'Probable AWS secret access key in source. Rotate the credential and load it from a secret manager / env var.',
  ...
  for (const hit of matchInAddedLines(diff, PRIVATE_KEY_BLOCK_RE)) {
    findings.push(build({
      ...
      confidence: 'high',
      description:
        'Private key material committed to source. Remove immediately, rotate the key, and store it in a secret manager.',
    }));
  }
  ...
  for (const hit of matchInAddedLines(diff, BEARER_TOKEN_RE)) {
    findings.push(build({
      ...
      confidence: 'high',
      description:
        'Hardcoded Bearer token in a string literal. Rotate the token and read it from an environment variable at runtime.',
  ...
  for (const hit of matchInAddedLines(diff, SLACK_TOKEN_RE)) {
    findings.push(build({
      ...
      confidence: 'high',
      description:
        'Hardcoded Slack token. Revoke via Slack app admin and move to a secret manager.',
  ...
  for (const hit of matchInAddedLines(diff, GITHUB_TOKEN_RE)) {
    findings.push(build({
      ...
      confidence: 'high',
      description:
        'Hardcoded GitHub token. Revoke the PAT / app token immediately and read it from env or a secret manager.',
  ...
  for (const hit of matchInAddedLines(diff, STRIPE_KEY_RE)) {
    findings.push(build({
      ...
      confidence: 'high',
      description:
        'Hardcoded Stripe API key. Roll the key in the Stripe dashboard and load it from an environment variable.',
  ...
  for (const hit of matchInAddedLines(diff, GENERIC_SECRET_RE)) {
    ...
    findings.push(build({
      ...
      description:
        `Possible hardcoded ${hit.match[1]} assignment. If this is a real credential, rotate it and load from an env var. If it is a test fixture, mark it as such and avoid realistic-looking values.`,
  ...
  for (const hit of matchInAddedLines(diff, EVAL_CALL_RE)) {
    findings.push(build({
      ...
      confidence: 'med',
      description:
        '`eval()` executes arbitrary code and is a common RCE vector. Parse structured data (JSON) or use a safe evaluator instead.',
    }));
  }
  ...
  for (const hit of matchInAddedLines(diff, NEW_FUNCTION_RE)) {
    findings.push(build({
      ...
      confidence: 'med',
      description:
        '`new Function(...)` compiles a string as code — equivalent to `eval`. Avoid unless you fully control the input.',
    }));
  }
  ...
  for (const hit of matchInAddedLines(diff, CMD_INJECTION_RE)) {
    findings.push(build({
      ...
      confidence: 'high',
      description:
        'Command execution with interpolated / concatenated input is a classic shell injection vector. Pass arguments as an array to spawn() and never build a command string from user input.',
    }));
  }
  ...
  for (const hit of matchInAddedLines(diff, SQL_INJECTION_RE)) {
    findings.push(build({
      ...
      confidence: 'high',
      description:
        'SQL query built via string concatenation or template interpolation. Use parameterised queries / prepared statements to prevent SQL injection.',
    }));
  }
  ...
  for (const hit of matchInAddedLines(diff, DANGEROUS_INNER_HTML_RE)) {
    findings.push(build({
      ...
      confidence: 'med',
      description:
        '`dangerouslySetInnerHTML` bypasses React\'s XSS protections (OWASP A03). Sanitize with DOMPurify or render trusted content through regular JSX.',
    }));
  }
  ...
  for (const hit of matchInAddedLines(diff, DOCUMENT_WRITE_RE)) {
    findings.push(build({
      ...
      confidence: 'med',
      description:
        '`document.write` can introduce XSS and breaks CSP. Use DOM APIs or framework rendering instead.',
    }));
  }
  ...
  for (const hit of matchInAddedLines(diff, WEAK_HASH_RE)) {
    ...
    findings.push(build({
      ...
      confidence: 'med',
      description:
        `Weak hash algorithm "${algo}" — broken for collision resistance. Use sha256 or better; for password hashing use bcrypt/argon2/scrypt.`,
    }));
  }

  for (const hit of matchInAddedLines(diff, MATH_RANDOM_RE)) {
    if (!MATH_RANDOM_CONTEXT_RE.test(hit.text)) continue;
    findings.push(build({
      file: hit.file,
      line: hit.lineNumber,
      text: hit.text,
      ...
      description:
        '`Math.random()` is not cryptographically secure. Use `crypto.randomBytes` or `crypto.randomUUID` when generating tokens, keys, nonces, or salts.',
    }));
  }
  ...
  for (const hit of matchInAddedLines(diff, PSEUDO_RANDOM_RE)) {
    findings.push(build({
      ...
      confidence: 'med',
      description:
        '`crypto.pseudoRandomBytes` is not cryptographically strong — use `crypto.randomBytes` instead.',
    }));
  }
  ...
  for (const hit of matchInAddedLines(diff, OPEN_REDIRECT_RE)) {
    findings.push(build({
      ...
      confidence: 'high',
      description:
        'Redirect target is user-controlled — classic open-redirect pattern (phishing vector). Validate the target against an allow-list of known safe URLs / paths before redirecting.',
    }));
  }
  ...
  for (const hit of matchInAddedLines(diff, CORS_WILDCARD_RE)) {
    findings.push(build({
      ...
      confidence: 'med',
      description:
        'Wildcard `Access-Control-Allow-Origin: *` disables origin restriction. Prefer an explicit allow-list; `*` is incompatible with credentialed requests anyway.',
    }));
  }
