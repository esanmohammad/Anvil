...
function isTestPath(path) {
    ...
    if (base.includes('.test.') || base.includes('.spec.'))
        return true;
    if (/(^|\/)__tests__\//.test(path.replace(/\\/g, '/')))
        return true;
...
function* walkTestFiles(root) {
    ...
    while (stack.length > 0) {
        ...
        for (const name of entries) {
            ...
            if (dot === -1 || !TEST_EXTS.has(name.slice(dot)))
                continue;
            if (!isTestPath(p))
                continue;
            yield p;
 ...
 */
export function checkTestExists(finding, repoLocalPath) {
...
//# sourceMappingURL=test-exists-check.js.map