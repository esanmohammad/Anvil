...
function hasSymbol(haystack, symbol) {
    if (!symbol)
        return false;
 ...
 */
export function checkSymbolExists(finding, repoLocalPath, fileContent) {
    ...
    for (let level = 0; level < 3; level++) {
        ...
        if (parent === currentDir)
            break;
        if (parent.length < repoRoot.length)
            break;
        currentDir = parent;
    }
    return {
