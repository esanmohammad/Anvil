    ...
    while (stack.length > 0) {
        ...
        for (const name of entries) {
            ...
            if (st.isDirectory()) {
                ...
                if (dot === -1 || !CODE_EXTS.has(name.slice(dot)))
                    continue;
                yield p;
                scanned++;
...
function countOccurrencesIn(text, needle) {
    if (!needle)
        return 0;
    let count = 0;
    let idx = 0;
 ...
 */
export function checkPrecedent(finding, repoLocalPath, opts = {}) {
    ...
    for (const file of walk(root)) {
        ...
        if (total >= minPrecedents + 1)
            break; // early-out once we're certain
    }
...
//# sourceMappingURL=precedent-check.js.map