...
function claimIsNullOrType(finding) {
    return finding.claimType === 'null-deref' || finding.claimType === 'type-mismatch';
}
function fileHasDiagnostic(output, filePath, line) {
    ...
    for (const l of lines) {
        ...
        if (Math.abs(reported - line) <= 1)
            return true;
    }
    return false;
}
async function checkTs(finding, repoLocalPath, filePath) {
...
function checkPy(finding, repoLocalPath, filePath) {
...
function checkGo(finding, repoLocalPath, filePath) {
    const result = runCmd('go', ['vet', './...'], repoLocalPath);
    if (!result.ok)
        return { passed: true, detail: 'skipped: go toolchain not available' };
    const combined = `${result.stdout}\n${result.stderr}`;
    if (result.code === 0) {
 ...
 */
export async function checkTypeClaim(finding, repoLocalPath, filePath) {
    ...
    if (lang === 'ts')
        return checkTs(finding, absRepo, filePath);
    if (lang === 'py')
        return checkPy(finding, absRepo, filePath);
