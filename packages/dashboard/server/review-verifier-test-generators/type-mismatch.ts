...
export function generateTypeMismatchTest(
  finding: unknown,
  language: VerifierLanguage,
  opts: TypeMismatchOpts,
): MicroTest | null {
  void finding;
 ...
 */
export function interpretTscOutput(
  stdout: string,
  stderr: string,
  expectedType?: string,
): { reproduced: boolean; evidence: string } {
  const combined = `${stdout}\n${stderr}`;
  const evidence = combined.split('\n').filter((l) => /error TS\d+/i.test(l)).slice(0, 3).join('\n');
  if (!evidence) return { reproduced: false, evidence: combined.slice(0, 400) };
