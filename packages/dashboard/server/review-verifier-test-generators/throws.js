...
export function claimMentionsThrow(message) {
    if (typeof message !== 'string')
        return false;
    return THROW_WORDS.test(message);
}
export function generateThrowsTest(finding, language, functionName) {
    void finding;
...
function buildNodeThrowsTest(functionName, lang) {
...
function buildPyThrowsTest(functionName) {
    const source = `# Anvil R3 micro-test — throws probe
import sys
...
function buildGoThrowsTest(functionName) {
    const source = `// Anvil R3 micro-test — throws probe
package main
