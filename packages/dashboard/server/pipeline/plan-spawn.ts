...
export interface PlanSpawnDeps {
  ...
  lifecycle: Pick<
    PlanLifecycleHandle,
    'dispatchLifecycle' | 'isPartOfActiveRefine' | 'noteRefineRegenCompleted'
  >;
}
...
export function createPlanSpawn(deps: PlanSpawnDeps): PlanSpawnHandle {
  const planAgentContext = new Map<string, PlanAgentContextEntry>();
...
Reply with ONLY a fenced \`\`\`json ... \`\`\` block containing the updated section value (matching that section's schema — an object for scope/architecture/rollout/tests/estimate, an array for repos/contracts/risks, or a string for problem). No prose.`;
  ...
  // ── Spawners ─────────────────────────────────────────────────────
  function resolveCwd(project: string): string {
    const configWorkspace = deps.getWorkspaceFromConfig(project);
    if (configWorkspace && existsSync(configWorkspace)) return configWorkspace;
    return join(
  ...
  function loadKbReport(project: string, query: string): string {
  ...
  function spawnPlanAgent(
    ...
    if (!retryState) deps.resetOutputBuffer();
    ...
    deps.services.agents.emit('agent.spawned', { ...agent, runId } as never);
  }
  ...
  function spawnPlanSectionRegen(
    ...
    const basePrompt = buildPlanPrompt(
      existingPlan.project,
      existingPlan.feature,
      repoNames,
      kbReport,
      section,
      existingPlan,
  ...
  function finalizePlanAgent(agentId: string, agentOutput: string): void {
    ...
    try {
      if (ctx.existingSlug && ctx.section) {
        ...
        deps.services.plans.emit('plan.updated', { plan: next, validation, section: ctx.section } as never);
        // Lifecycle dispatch:
        ...
        deps.services.plans.emit('plan.variant-created', { plan, validation, variant: ctx.variant } as never);
        ...
        const validation = deps.planValidator.validate(plan);
        deps.planStore.writeValidation(ctx.project, plan.slug, validation);
        deps.services.plans.emit('plan.created', { plan, validation } as never);
        void deps.lifecycle.dispatchLifecycle(ctx.project, plan.slug, { kind: 'plan-draft-complete' });
        void deps.lifecycle.dispatchLifecycle(ctx.project, plan.slug, {
          kind: 'verify-complete',
          errors: validation.counts.errors,
