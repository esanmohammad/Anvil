...
import { InMemoryEventBus, attachAuditLogHook, attachCostTrackerHook, attachDashboardStateHook, attachLearnersHook, } from '@esankhan3/anvil-core-pipeline';
import { attachPipelineBusSubscriber, } from '../pipeline-bus-subscriber.js';
...
import { computeKey as computeCheckpointKey, } from '@esankhan3/anvil-agent-core';
...
export function createStartPipeline(deps) {
    return function startPipeline(project, feature, options) {
        ...
        }, deps.memoryStore, deps.kbManager);
        ...
        const stepDescriptors = initialState.stages.map((s) => ({
            id: s.name,
            name: s.name,
            label: s.label,
            perRepo: s.perRepo,
        ...
        const learnersHook = attachLearnersHook(pipelineBus, {
            ...
            onLearnEvent: (event) => {
                ...
                if (payload?.state)
                    autoLearn(deps.memoryStore, payload.state);
            },
        });
        const busSubscriber = attachPipelineBusSubscriber(pipelineBus, {
            ...
            broadcast: (msg) => {
                if (msg.type === 'state' && msg.payload && typeof msg.payload === 'object') {
                    deps.services.system.emit('state', { state: msg.payload });
                }
            },
        });
        const detachBus = () => {
        ...
        // ── Cost ledger hook — gated per-project by policy.cost in pipeline-policy.yaml ──
        {
            deps.agentManager.setCostHook((info) => {
                ...
                if (!policy?.cost)
                    return;
                const stage = (['plan', 'implement', 'review', 'test', 'ship'].includes(info.stage ?? '')
                    ? info.stage
                    : 'other');
                try {
                ...
                if (info.project && info.runId) {
                    try {
                        deps.broadcastCostSnapshot(info.project, info.runId);
                    }
                    catch { /* ok */ }
                }
            });
        ...
        if (process.env.ANVIL_CHECKPOINTS_ENABLED === '1') {
            ...
            const similarityIndices = new Map();
            const getSimilarityIndex = (p) => {
                ...
                lookup: (input) => {
                    try {
                        ...
                        if (similarityEnabled) {
                            ...
                            const match = getSimilarityIndex(input.project).nearest({ runFamily, stage, taskId, model: input.model, promptVersion }, vec, similarityThreshold);
                            if (match) {
                                const blob = deps.blobStore.read(match.entry.outputRef);
                                if (blob) {
                                    process.stderr.write(`[checkpoint-similarity] hit project=${input.project} stage=${stage} ` +
        ...
        runner.on('user-input', ({ stageIndex, text }) => {
        ...
        runner.on('project-event', (data) => {
            ...
            const entry = {
                ...
                type: (data.level === 'warn' ? 'stderr' : 'stdout'),
        ...
        runner.on('pipeline-complete', (pipelineState) => {
            ...
            if (prUrls.length && options?.planSeed) {
                const personas = ['architect', 'security', 'tester'];
        ...
        runner.on('pipeline-fail', (pipelineState) => {
            deps.persistRunRecord(pipelineState, pipelineRunId);
            autoLearn(deps.memoryStore, pipelineState);
            deps.setActivePipelineRunner(null);
...
//# sourceMappingURL=start-pipeline.js.map