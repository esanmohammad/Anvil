...
import type { ActiveRun, ActiveRunStage, ActivityEntry } from '../broadcasts.js';
...
export function createQuickActionSpawner(
  ...
  return function spawnQuickAction(actionType, project, description, model) {
    ...
    if (kbReport) {
      ...
      deps.services.agents.emit('agent.output', { entries: [kbEntry], runId } as never);
    } else {
      const kbEntry: ActivityEntry = {
        timestamp: Date.now(),
        stage: actionLabel,
    ...
    deps.services.agents.emit('agent.output', { entries: [wsEntry], runId } as never);
...
You have ${repoNames.length} repos: ${repoNames.join(', ')}. Stay within these directories only.\n\n` : ''}This is read-only research — do NOT modify any files.`,
    ...
    const stageMap: Record<QuickActionType, string> = {
      'run-fix': 'fix',
      'run-review': 'review',
      'run-spike': 'research',
    };
    ...
    if (actionType === 'run-fix') {
      ...
      runFixFlow({
        ...
        onSpawn: (stage, _repo, agentId) => {
          ...
          deps.services.agents.emit('agent.spawned', { id: agentId, runId, stage } as never);
        },
      })
        .then((result) => {
          const run = deps.activeRuns.get(runId);
