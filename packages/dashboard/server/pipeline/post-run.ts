...
export interface PostRunDeps {
  ...
  activeRuns: Map<string, { prUrls: Set<string> }>;
  /** Resolve `<workspaceRoot>/<project>` for the reflection invoker cwd. */
  getWorkspaceFromConfig: (project: string) => string | null;
}

export function createPostRunPersister(
  deps: PostRunDeps,
): (state: PipelineRunState, runId?: string) => Promise<void> {
  return async function persistRunRecord(state, runId) {
    ...
    try {
      if (!existsSync(deps.runsDir)) mkdirSync(deps.runsDir, { recursive: true });
