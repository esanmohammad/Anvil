...
export function createReviewSpawn(deps: ReviewSpawnDeps): ReviewSpawnHandle {
  const reviewAgentContext = new Map<string, ReviewAgentContextEntry>();
  ...
  async function startReviewRun(
    ...
    try { recordReviewCreated(deps.anvilHome, project); } catch { /* ok */ }

    deps.services.reviews.emit('review.started', { reviewId: prId, prId, personas, project } as never);
    ...
    try {
      const convFindings = runConventionRules(
        ...
        { anvilHome: deps.anvilHome, project },
      );
    ...
    for (const persona of personas) {
      ...
      deps.services.agents.emit('agent.spawned', { ...agent, reviewId: prId, persona } as never);
    }
  }
  ...
  async function finalizeReviewAgent(agentId: string, agent: AgentState): Promise<void> {
    ...
    if (process.env.ANVIL_REVIEW_VERIFIER === 'on' && ctx.repoLocalPath && ctx.fileContents) {
      try {
        ...
        const keptIds = new Set((v.verified as Array<{ id?: string }>).map((f) => f.id));
        filteredFindings = filteredFindings.filter((f) => keptIds.has(f.id));
    ...
    try {
      ...
      if (final) {
        ...
        if (!anyStillRunning) {
          deps.services.reviews.emit('review.created', { review: final } as never);
          ...
          if (process.env.ANVIL_REVIEW_PUBLISH === 'on' && final.pr?.url) {
            (async () => {
              try {
                ...
                deps.services.reviews.emit('review.published', { reviewId: ctx.reviewId, ...result } as never);
              } catch (err) {
  ...
  async function applyReviewFix(project: string, reviewId: string, findingId: string): Promise<string> {
    ...
    if (updated) {
      ...
      if (finding2) {
        try { recordResolution(deps.anvilHome, project, updated, finding2, priorResolution); } catch { /* ok */ }
      }
    }
