 ...
 */
export function largestBalancedSpan(
  text: string,
  open: string,
  close: string,
): string | null {
...
export function extractJsonBlockFromText(text: string): unknown | null {
  const fenceRe = /```json\s*([\s\S]*?)```/gi;
 ...
 * are objects in JS.
 */
export function isValidParsedShape(parsed: unknown, section?: PlanSection): boolean {
 ...
 */
export function buildJsonCorrectionInput(
  badOutput: string,
  section?: PlanSection,
): string {
