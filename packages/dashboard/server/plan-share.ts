/**
 * plan-share — HMAC-signed share tokens for read-only plan URLs.
 *
 * A share token is an opaque, signed, self-contained capability that lets a
 * third party view a specific (project, slug, version) tuple of a plan for a
 * bounded window without authenticating to the dashboard. The token embeds
 * its own expiry and is verified entirely from the shared secret — no
...
export interface SharePayload {
  project: string;
  slug: string;
  version: number;       // freeze to a specific version
  expiresAt: number;     // unix ms
}

export const SHARE_TOKEN_TTL_MS = 7 * 24 * 3600 * 1000; // 7 days
...
/**
 * Read ~/.anvil/.share-secret, or generate a fresh 32-byte hex secret,
 * persisted with mode 0600 so only the owner can read it.
 */
export function getOrCreateShareSecret(anvilHome: string): string {
  const secretPath = join(anvilHome, '.share-secret');
