...
`;
export async function extract(args, opts) {
    const stage = 'browser-extractor';
    const userPrompt = buildUserPrompt(args, opts.pageText);
    const { answer, model } = await runWithChainFallback({
        stageName: stage,
        maxAttempts: 3,
        resolveModel: () => {
...
function buildUserPrompt(args, pageText) {
    const lines = [
...
function parseExtractAnswer(answer) {
