...
function Panel({ title, description, children, dimmed, comingSoon }: {
  title: string;
  description?: string;
  ...
  return (
    <section className="card" style={{
      marginBottom: 12,
      padding: 16,
      opacity: dimmed || comingSoon ? 0.55 : 1,
      position: 'relative',
    }}>
      ...
      <div style={{ pointerEvents: comingSoon ? 'none' : 'auto' }}>
        {children}
      </div>
    </section>
  );
...
function CostPanel({ policy }: { policy: PolicyHook }): React.ReactElement {
  ...
  return (
    <Panel title={POLICY_COPY.cost.title} comingSoon>
      <div style={{ marginBottom: 10 }}>
...
function NotificationsPanel({ policy }: { policy: PolicyHook }): React.ReactElement {
  return (
    <Panel title={POLICY_COPY.notifications.title} comingSoon>
